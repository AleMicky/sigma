...
public interface SpringGrupoAprobadorDependienteRepository
    ...
    Page<GrupoAprobadorDependienteEntity> findByGrupoAprobadorId(UUID grupoAprobadorId, Pageable pageable);
    ...
    @Query("""
                SELECT DISTINCT new com.endecorani.sigma_api.modules.organizacion.application.dto.response.DependienteSelectResponse(
                    e.id,
                    TRIM(CONCAT(COALESCE(p.nombres, ''), ' ', COALESCE(p.primerApellido, ''), ' ', COALESCE(p.segundoApellido, ''))),
                    c.nombre
                )
                ...
                WHERE e.activo = true
                  AND e.id IN (
                      SELECT dep.empleadoId
                      FROM GrupoAprobadorDependienteEntity dep
                      WHERE dep.grupoAprobadorId IN (
                          SELECT d.grupoAprobadorId
                          FROM GrupoAprobadorDetalleEntity d
                          WHERE (d.tipoAprobador = 'EMPLEADO' AND d.empleadoId = :aprobadorId)
                             OR (d.tipoAprobador = 'CARGO' AND d.cargoId = (
                                 SELECT emp.cargoId FROM EmpleadoEntity emp WHERE emp.id = :aprobadorId
                             ))
                      )
                  )
            """)
    List<DependienteSelectResponse> findDependientesSelectByAprobadorId(@Param("aprobadorId") UUID aprobadorId);
}
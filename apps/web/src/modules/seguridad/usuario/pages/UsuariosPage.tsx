...
export function UsuariosPage() {
  ...
  const syncMutation = useSincronizarUsuarios()
  ...
  useClampPage(
    search.page,
    search.setPage,
    usuariosQuery.data?.totalPages,
  )

  const stats = useMemo(() => {
    const total = usuariosQuery.data?.totalElements ?? 0
    const activos = usuarios.filter((u) => u.activo).length
    const inactivos = usuarios.filter((u) => !u.activo).length
    return { total, activos, inactivos }
  }, [usuariosQuery.data?.totalElements, usuarios])
    ...
    <PageShell className="h-full min-h-0 w-full max-w-none gap-0 overflow-hidden px-4 py-0 sm:px-6 md:px-8 lg:px-10 md:py-0">
      ...
      <header className="flex shrink-0 flex-col gap-3 border-b py-4 sm:gap-4 sm:py-6 md:flex-row md:items-start md:justify-between md:py-8">
        <div className="min-w-0 flex flex-1 flex-col gap-1.5">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-1.5 shrink-0 md:hidden">
              <RefreshButton
                ...
                isRefreshing={usuariosQuery.isFetching}
              />
              <Button
                size="sm"
                variant="outline"
          ...
          <p className="text-sm text-muted-foreground">
            Gestión y sincronización de usuarios locales con el servidor de identidades Keycloak.
          </p>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        {usuariosQuery.isLoading ? (
          ...
          <EmptyState
            title={getErrorMessage(usuariosQuery.error)}
          ...
          <EmptyState
            ...
            description={
              search.search.trim()
                ...
                : "Ejecuta la sincronización con Keycloak para cargar los usuarios registrados en el sistema."
            }
            action={
          ...
          <>
            <div
              className={cn(
                ...
                usuariosQuery.isFetching && "opacity-70",
              )}
            >
      ...
      {/* Detalle Modal */}
      <UsuarioDetailDialog
        key={selectedUsuario?.id ?? "empty-detail"}
